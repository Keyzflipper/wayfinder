...
// /api/restaurants, /api/describe, /api/guide (upload), /api/guide/nearby
// (lookup), /api/trips, /api/finds, and /api/photos.
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
      ...
      if (url.pathname === '/api/describe' && request.method === 'GET') {
        return withCors(await handleDescribe(request, env));
      }
